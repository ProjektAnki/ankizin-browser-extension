// Ankizin Content Copier Firefox Extension
...
class AnkizinCopier {
  ...
  createFallbackButton(hostname) {
    ...
    button.innerHTML = `
      <img src="${browser.runtime.getURL('images/ankizin.svg')}" alt="Ankizin" class="ankizin-icon">
  ...
  createIcon() {
    ...
    icon.src = browser.runtime.getURL('images/ankizin.svg');
    icon.alt = 'Ankizin';
  ...
  async copyId(id, button) {
    ...
    try {
      // Firefox supports navigator.clipboard in newer versions
      if (navigator.clipboard && navigator.clipboard.writeText) {
        await navigator.clipboard.writeText(searchString);
      } else {
        // Fallback for older Firefox versions
        this.fallbackCopyToClipboard(searchString);
      }
